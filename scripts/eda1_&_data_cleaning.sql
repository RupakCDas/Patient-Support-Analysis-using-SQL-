...
/*creating a staging table. This is the one I will work in and 
clean the data. I want a table with the raw data in case something happens */

CREATE TABLE united_health.data_b2
LIKE united_health.data_b;
...
SELECT * FROM (
  	        SELECT policy_holder_id, case_id, call_category, call_date, call_duration_secs,
  		      ROW_NUMBER() OVER (
  			           PARTITION BY policy_holder_id, case_id, call_category, call_date, call_duration_secs
  			           ) AS row_num
...
WITH DELETE_CTE AS (
         ...
         FROM (
	             ...
	             FROM  united_health.data_b2 ) duplicates
          WHERE row_num > 1;
)
